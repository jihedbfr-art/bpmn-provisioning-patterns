...
public class OutboxRelay {
     ...
     */
    // The schedule and the bean are separated on purpose, and this merge kept that over main's
    // version. On main the relay carries @ConditionalOnProperty, so relay.enabled=false removes
    // the bean itself — which is why OutboxRelayIT there had to keep the bean alive by pushing the
    // first run an hour out instead of simply disabling it. Here the property means what it says:
    // do not run on a schedule. The bean stays, and a test can drive publishBatch() directly.
