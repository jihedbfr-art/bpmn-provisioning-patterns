...
Ce dépôt est une implémentation de référence exécutable de patterns d'orchestration sur Camunda, Spring Boot et Kafka. Il est fait pour être lu et lancé en local contre l'environnement Docker fourni, pas déployé tel quel. Les identifiants présents dans les fichiers compose sont des valeurs par défaut locales et ne constituent pas une vulnérabilité.

## Versions supportées

Seule la branche `main` est maintenue. Il n'y a aucun artefact publié.
